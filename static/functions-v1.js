...
var samplerate = 135; //PPG maximum in SDK 28Hz, 44Hz, 135Hz, 176Hz
dataobject = {};
layoutCombined = [];
obj = [];
plotlyOut = 0;
...
function printHeartRate(event) {
  ...
  if (DataType == 2) {
    //accelerometer
    frame_type = Number(new Uint8Array(data.slice(9, 10)));
    ...
    if (frame_type == 1) {
      ...
      for (offset = 0; offset < npoints; offset += 3) {
        i = offset / 3;
        ACC[0][i] = samples[offset];
        ACC[1][i] = samples[offset + 1];
        ACC[2][i] = samples[offset + 2];
      }
      ACCtime = fillTimeArray(
        DataType,
        devicename,
        data.slice(1, 9),
        dataTime,
    ...
    for (i = 0; i < ACC[0].length; i++) {
      ACC[3][i] = Math.round(
        (Math.sqrt(ACC[0][i] ** 2 + ACC[1][i] ** 2 + ACC[2][i] ** 2) / 16) *
          200,
      );
    } //scaled by 20 to display on PPG graph
    pushData(devicename, ["acc_x", "acc_y", "acc_z", "acc_v"], ACCtime, ACC);
    resolution =
      (layoutCombined[devicename].yaxis.range[1] -
        layoutCombined[devicename].yaxis.range[0]) /
      (document.getElementById("Combined").clientHeight - 100);
    ReduceOld(devicename, dataobject, "acc_v", "acc_v_reduced", resolution);
  }
  //ECG
  if (DataType == 0) {
    //ECG
